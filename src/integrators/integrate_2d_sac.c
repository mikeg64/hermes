...
#ifdef MHD
static Real ***B1_x1=NULL, ***B2_x2=NULL, ***B3_x3=NULL;
 ...
 */
    for (i=il+1; i<=iu; i++) {
      Uc_x1[j][i] = Prim1D_to_Cons1D(&W[i],&Bxi[i]);
      

#ifdef MHD
      Bx = B1_x1[j][i];
#endif
      fluxes(Uc_x1[j][i],Uc_x1[j][i],W[i],W[i],Bx,&x1Flux[j][i]);
    }
  }
 ...
 */
    ...
    for (j=jl+1; j<=ju; j++) {
      Uc_x2[j][i] = Prim1D_to_Cons1D(&W[j],&Bxi[j]);
      
#ifdef MHD
      Bx = B2_x2[j][i];
#endif
      fluxes(Uc_x2[j][i],Uc_x2[j][i],W[j],W[j],Bx,&x2Flux[j][i]);
    }
  }
  ...
  for (k=ks-1; k<=ke+1; k++) {
    for (j=js-1; j<=je+1; j++) {
      for (i=is; i<=ie+1; i++) {
        ...
        Bx = B1_x1Face[k][j][i];
#endif
        W[i] = Cons1D_to_Prim1D(&Ul_x1Face[k][j][i],&Bx);
       

        fluxes(Ul_x1Face[k][j][i],Ul_x1Face[k][j][i],W[i],W[i],Bx,
               &x1Flux[k][j][i]);
      }
    }
  }
  ...
  for (k=ks-1; k<=ke+1; k++) {
    for (j=js; j<=je+1; j++) {
      for (i=is-1; i<=ie+1; i++) {
        ...
        Bx = B2_x2Face[k][j][i];
#endif
        W[i] = Cons1D_to_Prim1D(&Ul_x2Face[k][j][i],&Bx);
        

        fluxes(Uc[k][j][i],Uc[k][j][i],W[i],W[i],Bx,
               &x2Flux[k][j][i]);
      }
    }
  }
...
hyperdifvisc1il
		         ...
		         for(ii1=0;ii1<=1;ii1++)
		         {
				  ...
				  if(ii==dim)
				    hyperdifbsource1(ii,ii0,p->dt);
				  else
				    hyperdifbsourcene1(ii,ii0,p->dt);  //off diagonal
		        }
