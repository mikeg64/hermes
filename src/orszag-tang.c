...
/* Calculate the cell center positions */
      ...
      pGrid->U[j][i].d = d0;
      pGrid->U[j][i].M1 = -d0*v0*sin(2.0*PI*x2);
      pGrid->U[j][i].M2 =  d0*v0*sin(2.0*PI*x1);
      pGrid->U[j][i].M3 = 0.0;
...
/* initialize total energy and cell-centered B */
  ...
  for (j=js; j<=je; j++) {
  for (i=is; i<=ie; i++) {
    pGrid->U[j][i].B1c = 0.5*(pGrid->B1i[j][i]+pGrid->B1i[j][i+1]);
    pGrid->U[j][i].B2c = 0.5*(pGrid->B2i[j][i]+pGrid->B2i[j+1][i]);
    pGrid->U[j][i].B3c = 0.0;
#ifndef ISOTHERMAL
    pGrid->U[j][i].E = p0/Gamma_1
